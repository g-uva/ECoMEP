...
def main():
    ...
    # Load alias map and params (schema requirements & ranges are optional)
    aliases = {}
    ...
    # Apply simple alias remapping: first available alias wins
    alias_map = {}
    for canon, candidates in (aliases.get("aliases", {}) or {}).items():
        for cand in candidates:
            if cand in df.columns:
                alias_map[cand] = canon
                break
    for src, dst in list(alias_map.items()):
